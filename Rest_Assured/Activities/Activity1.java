...
public class Activity1 {
	...
	public void addNewPet() {
		...
		// Assertion
		response.then().body("id", equalTo(77991));
		response.then().body("name", equalTo("Pulto"));
	...
	public void getPetInfo() {
		Response response = given()
			.baseUri("https://petstore.swagger.io/v2/pet") // Set base URI
			.header("Content-Type", "application/json") // Set headers
			.when().pathParam("petId", 77991) // Set path parameter
	...
	public void deletePet() {
		...
		response.then().body("message", equalTo("77991"));
	}
}